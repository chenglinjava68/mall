...
package com.plateno.booking.internal.interceptor.web.method.support;
	...
	public Object invokeForRequest(NativeWebRequest request, ModelAndViewContainer mavContainer, Object... providedArgs) throws Exception {
		...
		if (returnValue instanceof ResultVo) {
			...
			if (resultVo.getResultCode().startsWith(BaseReslutCodeConstants.CODE_SYSTEM_ERROR)) {
				...
				logger.info("请求方法:" + methodName + "系统异常");
			}
		}
		long endTime = System.currentTimeMillis();

		logger.info("RA:" + runningAccount + " " + "Method [" + getMethod().getName() + "] " + AdamSysConstants.LINE_SEPARATOR + "returned [" + gson.toJson(returnValue) + "]" + "useTime:" + (endTime - beginTime));
		return returnValue;
	}
	...
	private Object[] getMethodArgumentValues(NativeWebRequest request, ModelAndViewContainer mavContainer, Object... providedArgs) throws Exception {
		...
		try {
			if (!StringUtils.isBlank(paramStr)) {
				...
				Gson gson = new Gson();
				args[0] = gson.fromJson(paramStr, getClass(parameters[0].getGenericParameterType()));
				return args;
			}
		} catch (Exception e) {
			// nothing to do
