package com.plateno.booking.internal.hotel.aspect;
...
public class LogAspect {
	...
	@Around("execution (* com.plateno.booking.internal.hotel.service.thirdService.impl.*Service.*(..))")
	public Object aroundMethod(ProceedingJoinPoint pjd) throws IOException {
		...
		try {
			...
			if ((end - begin) >= overtime) {
				...
				LogUtils.countTimeLoggerInfo("请求酒店外部接口服务：" + map);
			}else{
				LogUtils.countTimeLoggerInfo("请求酒店外部接口服务,耗费时间(毫秒)：" + decimalFormat.format(end - begin));
			}
		}
	}
}
